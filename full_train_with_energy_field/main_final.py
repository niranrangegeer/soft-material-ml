...
# 3、load images
# ⭐️⭐️⭐️ V7 关键修复：
# Train_images 是 3 面板, Test_images 是 5 面板。
# 我们需要两个不同的加载函数。
...
def load_3_panel(image_file):
    ...
    energy_image = image[:, w:2 * w, :]  # Panel 2: SED

    # "标准答案" (The Answer)
    real_image = image[:, 2 * w:, :]  # Panel 3: Crack path
...
def load_5_panel(image_file):
    ...
    # Panel 1: 初始几何
    input_image = image[:, :w, :]
    # Panel 2: 初始能量场 (SED PFM)
    energy_image = image[:, w:2 * w, :]

    # "标准答案" (The Answer)
    # Panel 3 (SED Prediction) - 被忽略
    # Panel 4 (Crack path PFM) - 这是我们的标准答案
    real_image = image[:, 3 * w:4 * w, :]
    # Panel 5 (Crack path Prediction) - 被忽略
...
image_file = PATH_train + 'Train_1.jpg'  # This line is just a placeholder, unused
...
def load_image_train(image_file):
    # ⭐️ V7 修复 3.3: 调用 3-panel 加载器
    input_image, energy_image, real_image = load_3_panel(image_file)
...
def load_image_test(image_file):
    # ⭐️ V7 修复 3.4: 调用 5-panel 加载器
    input_image, energy_image, real_image = load_5_panel(image_file)
...
BUFFER_SIZE = 120  # Per your request
BATCH_SIZE = 2
...
test_dataset = test_dataset.map(load_image_test)  # This now correctly uses load_5_panel
...
def Generator():
    ...
    # Concatenate inputs to feed into the network
    concatenated_input = tf.keras.layers.Concatenate()([inp, energy_inp])

    xe1 = concatenated_input
...
def Discriminator():
    ...
    # Discriminator accepts THREE inputs
    inp = tf.keras.layers.Input(shape=[IMG_WIDTH, IMG_WIDTH, 3], name='input_image')
    energy = tf.keras.layers.Input(shape=[IMG_WIDTH, IMG_WIDTH, 3], name='energy_image')
    tar = tf.keras.layers.Input(shape=[IMG_WIDTH, IMG_WIDTH, 3], name='target_image')  # target is Panel 3

    # Concatenate all THREE inputs (3+3+3=9 channels)
    x = tf.keras.layers.concatenate([inp, energy, tar])  # (bs, 256, 256, channels*3)
    ...
    # Model now accepts THREE inputs
...
def generator_loss(disc_generated_output, gen_output, target):
    ...
    # MAE
    # l1_loss compares gen_output (predicted crack) with target (real crack, Panel 3)
...
def generate_images(model, test_input, test_energy, tar, epoch, kj):
    # Pass BOTH inputs (Panel 1, Panel 2) to the model
    prediction = model([test_input, test_energy], training=True)

    # ⭐️ V7 修复: 重写绘图，使其 1x4 布局
    plt.figure(figsize=(20, 5))  # 调整 figsize 以适应 4 张图

    # 'test_input' 是 Panel 1 (初始几何)
    # 'test_energy' 是 Panel 2 (初始能量)
    # 'tar' 是 Panel 3 (真实裂纹) - 这是从 5-panel 图的 Panel 4 提取的
    # 'prediction' 是 (预测裂纹)

    display_list = [test_input[0], test_energy[0], tar[0], prediction[0]]
    title = ['Input Geometry (问题-几何)', 'Input Energy (问题-能量)',
             'Ground Truth (答案-裂纹)', 'Predicted Crack (答卷-裂纹)']

    for i in range(4):
        plt.subplot(1, 4, i + 1)  # ⭐️ 改成 1x4 布局
        plt.title(title[i])
...
def train_step(input_image, energy_image, target, epoch, n):
    with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
        # Pass BOTH inputs to the Generator
        gen_output = generator([input_image, energy_image], training=True)

        # Pass all THREE inputs to the Discriminator
        ...
        if n == 799:
            f = open(loss_file_path, 'a')  # Use the global loss_file_path
            f.write(
...
def fit(train_ds, epochs, test_ds, start_epoch):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        kj = 1
        for example_input, example_energy, example_target in test_ds.take(1):  # Changed from take(10) to take(1)
            generate_images(generator, example_input, example_energy, example_target, epoch + 1, kj)
            kj = kj + 1

        print(f"Epoch: {epoch} (Starts from {start_epoch})")  # Modified print
